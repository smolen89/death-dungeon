...
namespace RD.Util.RND
{
	...
	public class WeightedPool<T> : IWeightedPool<T>
	{
		...
		private List<WeightedItem<T>> pool = new List<WeightedItem<T>>();
		...
		{
			this.random = random ?? throw new ArgumentNullException( nameof(random), "Implementation of IRandom must not be null" );
